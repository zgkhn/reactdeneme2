...
export const authReducer = (state, action) => {
  ...
  switch (action.type) {
    case 'LOGIN':
      return { ...state, userNew: action.payload };
    case 'LOGOUT':
      return { ...state, userNew: null, authIsReady: false };
    case 'AUTH_IS_READY':
      return { ...state, 
...
;
    ...
    const unsub = onAuthStateChanged(auth, (userNew) => {
      dispatch({ type: 'AUTH_IS_READY', payload: userNew });
      if (userNew) {
        startLogoutTimer();
      } else {
        stopLogoutTimer();
